...
pme=get_me()
write(IDENTIFY,'(a,i3.3)') 'vecdump_me',pme
...
if(ind==0)then
  j=ind+100*i_color
  write(IDENTIFY,'(A,I3.3,A,I5.5)') 'cl',i_color,'lme',ind
  if(comp=='zz')then
    open(j,file="dsf_res_zz.dat"//trim(IDENTIFY),status='unknown')
  elseif(comp=='uu')then
    open(j,file="dsf_res_uu.dat"//trim(IDENTIFY),status='unknown')
  elseif(comp=='dd')then
    open(j,file="dsf_res_dd.dat"//trim(IDENTIFY),status='unknown')
  else
