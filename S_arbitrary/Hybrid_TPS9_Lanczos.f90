...
logical :: loop=.false.
double precision :: wk(max_itr,5)
integer :: iwk(max_itr)
!! initial setting
!$OMP PARALLEL
!$OMP WORKSHARE
wk(1:max_itr,1:5)=0d0
iwk(1:max_itr)=0
!$OMP END WORKSHARE
!$OMP END PARALLEL
!!
...
!!
  ...
  call interval_dump_read(position=progress, data_size=max_ldim, &
                          vec_size=max_ldim, sa=alpha, sb=beta ,sc=coef, se=ene,&
                          vec1=v0,vec2=v1)
...
!  write(*,'(A,I8,A,F12.4)') 'Itr :',i,'Time: ',tell_me_time()
...
!!
  ...
  alpha(i)=alpha1
  beta(i) =beta1
if(lme==0) write(*,*) 'ITR :',i,tell_me_time()
!!
!!
