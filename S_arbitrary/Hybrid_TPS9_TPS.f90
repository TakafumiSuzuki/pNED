...
use parameters, only : n, ldim, max_ldim, b_bit, p_bit, q_spin
...
!$OMP& FIRSTPRIVATE(qnorm,t_lme,bit_shift,ibit,bp,ss)
!$OMP do 
  do i=1, ldim
           istate=list1(i)
           is0=iand(istate,1)
    ...
    v(i)=v(i)+(dble(is)-ss)*(dble(is0)-ss)*dcmplx(dcos(qnorm),dsin(qnorm))*x(i)
  enddo
!$OMP END DO
!$OMP END PARALLEL
enddo
...
use data_mem, only : site_vec, list1, my_xor
use parallel_lib, only : lme, barrier
use parallelized_diag, only : calc_vec_inprdct
use openmp_lib
implicit none
...
!            stop
!  endif
  if(mod(i,1000)==1) call dump_alpha_beta_coef(dsf_comp,ind,i+1,v0,v1,vg)
