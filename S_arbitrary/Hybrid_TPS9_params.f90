...
!!
  integer :: simple=0
  integer :: c_count=3
  double precision :: xJ1   = 1d0,      xK1   = 1d0
  double precision :: xI11  = 0d0,      xI12  = 0d0
  double precision :: xJ1p  = 1d0,      xK1p  = 1d0
  double precision :: xJ1pp = 1d0,      xK1pp = 1d0
...
integer                     :: q_num=32 ! number of Q points
...
integer,          parameter :: max_itr=1000 ! Upper limit of Lanczos iterations
integer,          parameter :: s_maxitr=1000 ! Upper limit of DSF Lanczos iterations
double precision            :: localHam_max=1d2
!!
!!
!! progress; 1:lanc ene 2:lanc vec; 3:cg vec; 4:dsf; 5:recalc DSF, 6:extened (beta)
...
! for calc TPS
logical :: progress(7)=(/.false.,.false.,.false.,.false.,.false.,.false.,.true./)
!!
!! Computation time (default 23.5 h)
real :: max_comp_time=84000.0
character(LEN=2) :: dsf_comp='zz'
integer :: total_sz, ipm=0
integer :: n, ibond, Lx, Ly
integer :: q_spin, size_of_local_matrix
...
contains 
subroutine get_cline(d1,d2,d3,d4,d5,d6,d7,d8,d9,d10)
implicit none
...
real(KIND(0d0)), intent(inout), optional :: d5
integer, intent(inout), optional :: d6
character(LEN=2), intent(inout), optional :: d7
integer, intent(inout), optional :: d8
integer, intent(inout), optional :: d9
integer, intent(inout), optional :: d10
!!
integer :: numc
character(LEN=128) :: csim_time, clsize, clrange,  cold_np, cqstart, cqend, cpm, ctyp, cspin, crand
!!
numc=iargc()
!!
if(numc==10)then 
  ...
  call getarg(2,cspin)
  if(present(d2)) read(cspin,*) d2
  call getarg(3,clsize)
  if(present(d3)) read(clsize,*) d3
  call getarg(4,clrange)
  if(present(d4)) read(clrange,*) d4
  call getarg(5,cpm)
  if(present(d5)) read(cpm,*) d5
  call getarg(6,cold_np)
  if(present(d6)) read(cold_np,*) d6
  call getarg(7,ctyp)
  if(present(d7)) read(ctyp,*) d7
  call getarg(8,cqstart)
  if(present(d8)) read(cqstart,*) d8
  call getarg(9,cqend)
  if(present(d9)) read(cqend,*) d9
  call getarg(10,crand)
  if(present(d10)) read(crand,*) d10
else
  write(*,'(A)') 'ERROR: %s SIM_TIME [SPIN] [SIZE] [MODEL] [PARAM] [OLD_NP] [COMP] [Q_START Q_END] [RAND_INT]'
  write(*,'(A)') 'SPIN     : s=1/2<=2, s=1<=3,,, SPIN=2s+1'
  write(*,'(A)') 'SIM_TIME : upper limit of computational time'
  write(*,'(A)') 'SIZE     : system size'
  write(*,'(A)') 'MODEL    : see set_lattice subroutne. The integer varial "ind" corresponds to the value of MODEL.'
  write(*,'(A)') 'PARAM    : tuning parameter for each model'
  write(*,'(A)') 'OLD_NP   : number of flat mpi threads in the previous computation'
  write(*,'(A)') 'COMP     : "ee" for ground state calculation, "zz(xx)" for DSF calculations'
  stop
endif
write(*,'(F16.4,3I4,F8.2,I4,A,2I4,I8)') d1,d2,d3,d4,d5,d6,d7,d8,d9,d10
...
subroutine set_lattice(ind0,ind,pm)
  ...
  real(KIND(0d0)) :: pm
  size_of_local_matrix=q_spin*q_spin
  n=ind0
  total_sz=n/2 * (q_spin - 1) !! Total SZ=0
...
elseif(dsf_comp=='dd'.or.dsf_comp=='uu')then
...
elseif(dsf_comp=='zr')then
  ...
  dsf_comp='zz'
elseif(dsf_comp=='ur'.or.dsf_comp=='dr')then
  write(*,*) 'X component of DSF is reevaluated for matrix extention.'
  progress(:)=(/.false.,.false.,.false.,.true.,.false.,.true.,.false./)
  if(dsf_comp=='ur') dsf_comp='uu'
  if(dsf_comp=='dr') dsf_comp='dd'
elseif(dsf_comp=='tp')then
  write(*,*) 'Finite - T calculations'
...
ipm=0
if(dsf_comp=='uu') ipm=1
if(dsf_comp=='dd') ipm=-1
...
module working_area
use parameters, only : ldim, max_ldim,nvec, max_itr, s_maxitr
implicit none
...
allocate(alpha(max_itr), beta(max_itr), coef(max_itr,nvec+1))
allocate(salpha(1:s_maxitr),sbeta(1:s_maxitr))
wwk=0.d0
alpha=0.d0
beta=0.d0
coef=0.d0
...
complex(KIND(0d0)), allocatable :: xlocal_Ham(:,:,:)
logical,allocatable :: llocal_Ham(:,:,:)
logical,allocatable :: plocal_Ham(:,:)
complex(KIND(0d0)), allocatable :: my_xor(:,:)
...
integer :: L1, L2, L3, ic, c_count, iq, s, t
!!!!!
! Chain
!!!!
!c_count=2 ! cordination number (Maximun number of bonds connecting to the focusing site)
!
!pi=4d0*datan(1d0)
!k1p(:)=0d0
!k2p(:)=0d0
...
!      k2p(1)= 0d0                  ; k2p(2)= 0d0
!!!! Chain
!  ibond=c_count*n/2                               ! total number of bonds
!  q_num=n/2                                       ! total number of wave numbers for DSF calculation
!  call allocate_lattice_mem(n,ibond,q_num)
!  do i=1,n
!    site_vec(1,i)=dble(i);     site_vec(2,i)=0d0  ! positions of sites
!  enddo
!  do i=1,q_num
!    q_list(1,i)=k1(1)*dble(i); q_list(2,i)=0d0    ! wave number
!  enddo
!  do i=1,ibond
!    ipair(2*i-1)=i; ipair(2*i)=mod(i,n)+1         ! definition of site pair
!    ibond_type(i)=1                               ! type of interaction
!    bondwt(i)=1d0                                 ! bond weight
!  enddo
!  write(*,*) 'NUM BOND',ibond
!!!
!!!
!!!
!  Honeycomb lattice
...
complex(KIND(0d0)),allocatable :: sx(:,:),sy(:,:),sz(:,:)
complex(KIND(0d0)),allocatable :: su(:,:),sd(:,:)
complex(KIND(0d0)),allocatable :: sxx(:,:),sxy(:,:),sxz(:,:)
complex(KIND(0d0)),allocatable :: syx(:,:),syy(:,:),syz(:,:)
complex(KIND(0d0)),allocatable :: szx(:,:),szy(:,:),szz(:,:)
integer :: i, j, k, i1, j1, i2, j2
integer :: s1,s2,s3,s4
logical :: check
complex(KIND(0d0)) :: x1x,x1y,x1z,x2x,x2y,x2z,x3x,x3y,x3z
double precision :: xh11,xh12,xh13,xh2,xh3,rl,ss,sm
...
if(.not.allocated(xlocal_Ham))allocate(xlocal_Ham(0:size_of_local_matrix-1,0:size_of_local_matrix-1,9))
if(.not.allocated(llocal_Ham))allocate(llocal_Ham(0:size_of_local_matrix-1,0:size_of_local_matrix-1,9))
if(.not.allocated(plocal_Ham))allocate(plocal_Ham(0:size_of_local_matrix-1,9))
allocate(sx(0:q_spin-1,0:q_spin-1),sy(0:q_spin-1,0:q_spin-1),sz(0:q_spin-1,0:q_spin-1), &
         su(0:q_spin-1,0:q_spin-1),sd(0:q_spin-1,0:q_spin-1))
allocate(sxx(0:size_of_local_matrix-1,0:size_of_local_matrix-1), &
         syy(0:size_of_local_matrix-1,0:size_of_local_matrix-1), &
         szz(0:size_of_local_matrix-1,0:size_of_local_matrix-1), &
         sxy(0:size_of_local_matrix-1,0:size_of_local_matrix-1), &
         sxz(0:size_of_local_matrix-1,0:size_of_local_matrix-1), &
         syx(0:size_of_local_matrix-1,0:size_of_local_matrix-1), &
         syz(0:size_of_local_matrix-1,0:size_of_local_matrix-1), &
         szx(0:size_of_local_matrix-1,0:size_of_local_matrix-1), &
         szy(0:size_of_local_matrix-1,0:size_of_local_matrix-1), &
         my_xor(0:q_spin-1,0:q_spin-1) )
...
su(:,:)=dcmplx(0d0,0d0)
sd(:,:)=dcmplx(0d0,0d0)
my_xor(:,:)=dcmplx(0d0,0d0)

ss=0.5d0*dble(q_spin-1)
do i=0,q_spin-2
  sm=dble(i)-ss
  su(i,i+1)=dsqrt( (ss-sm)*(ss+sm+1d0) )
enddo
do i=1,q_spin-1
  sm=dble(i)-ss
  sd(i,i-1)=dsqrt( (ss+sm)*(ss-sm+1d0) )
enddo
do i=0,q_spin-1
  sz(i,i)=dble(i)-ss
enddo
...
sx(:,:)=dcmplx(0.5d0, 0d0)*(su(:,:)+sd(:,:))
sy(:,:)=dcmplx(0d0,-0.5d0)*(su(:,:)-sd(:,:))
my_xor(:,:)=(sx(:,:)+sy(:,:))

J1z(1,1)=pJ11     ; J1z(1,2)=pI111*rl ; J1z(1,3)=pI112*rl;
J1z(2,1)=pI111*rl ; J1z(2,2)=pJ11     ; J1z(2,3)=pI112*rl;
J1z(3,1)=pI112*rl ; J1z(3,2)=pI112*rl ; J1z(3,3)=pK11    ;

J1x(1,1)=pK12     ; J1x(1,2)=pI132*rl ; J1x(1,3)=pI122*rl;
J1x(2,1)=pI132*rl ; J1x(2,2)=pJ13     ; J1x(2,3)=pI121*rl;
J1x(3,1)=pI122*rl ; J1x(3,2)=pI121*rl ; J1x(3,3)=pJ12;

J1y(1,1)=pJ13     ; J1y(1,2)=pI132*rl ; J1y(1,3)=pI121*rl;
J1y(2,1)=pI132*rl ; J1y(2,2)=pK12     ; J1y(2,3)=pI122*rl;
J1y(3,1)=pI121*rl ; J1y(3,2)=pI122*rl ; J1y(3,3)=pJ12;
...
do j1 = 0, q_spin-1
do i1 = 0, q_spin-1
  sxx(q_spin*j1+i1,q_spin*j2+i2) = sx(i1,i2)*sx(j1,j2)
  syy(q_spin*j1+i1,q_spin*j2+i2) = sy(i1,i2)*sy(j1,j2)
  szz(q_spin*j1+i1,q_spin*j2+i2) = sz(i1,i2)*sz(j1,j2)
  sxy(q_spin*j1+i1,q_spin*j2+i2) = sx(i1,i2)*sy(j1,j2)
  syz(q_spin*j1+i1,q_spin*j2+i2) = sy(i1,i2)*sz(j1,j2)
  szx(q_spin*j1+i1,q_spin*j2+i2) = sz(i1,i2)*sx(j1,j2)
  syx(q_spin*j1+i1,q_spin*j2+i2) = sy(i1,i2)*sx(j1,j2)
  szy(q_spin*j1+i1,q_spin*j2+i2) = sz(i1,i2)*sy(j1,j2)
  sxz(q_spin*j1+i1,q_spin*j2+i2) = sx(i1,i2)*sz(j1,j2)
enddo
enddo
enddo
enddo
!!!!
!!!!
do i = 0, size_of_local_matrix-1
do j = 0, size_of_local_matrix-1
...
!!
     ...
     if(simple==3.or.simple==4)then
       ...
       x2x=dcmplx(0d0,0d0); x2y=dcmplx(0d0,0d0)
     endif
...
do k=1,9
do j=0,size_of_local_matrix-1
do i=j,size_of_local_matrix-1
  s1=mod(j,q_spin); s2=j/q_spin;
  s3=mod(i,q_spin); s4=i/q_spin;
  if (s1+s2 - (s3+s4) .ne. 0) then
    if( abs( dconjg(xlocal_Ham(i,j,k))*xlocal_Ham(i,j,k) )>eps ) parity_type=ibset(parity_type,0)
  endif
enddo
enddo
enddo
...
if(parity_type==0)then
  ...
  do j=0,size_of_local_matrix-1
  do i=0,size_of_local_matrix-1
!!    write(*,'(3I4,2e16.6E3)'),i,j,k,xlocal_Ham(i,j,k)
    s1=mod(j,q_spin); s2=j/q_spin;
    s3=mod(i,q_spin); s4=i/q_spin;
    if (s1+s2 - (s3+s4)==0) llocal_Ham(i,j,k)=.false.
    if(i==j) llocal_Ham(i,j,k)=.false.
  enddo
  enddo
  enddo
else
  ...
  do i=0,size_of_local_matrix-1
!!    write(*,'(3I4,2e16.6E3)'),i,j,k,xlocal_Ham(i,j,k)
    if( abs(dconjg(xlocal_Ham(i,j,k))*xlocal_Ham(i,j,k))>eps ) llocal_Ham(i,j,k)=.false.
    if(i==j) llocal_Ham(i,j,k)=.false.
  enddo
  enddo
  enddo
...
plocal_Ham(:,:)=.false.
do k=1,9
  do i2=0,size_of_local_matrix-1
    check=.true.
    do i1=0,size_of_local_matrix-1
      j1=i1
      j2=mod(i1+i2,size_of_local_matrix)
      if( abs(dconjg(xlocal_Ham(j1,j2,k))*xlocal_Ham(j1,j2,k))>eps ) check=.false.
    enddo
    if(.not.check) plocal_Ham(i2,k)=.true.
  enddo
enddo
deallocate(sx,sy,sz,su,sd,sxx,sxy,sxz,syx,syy,syz,szx,szy,szz)
...
integer :: i,j,k
double precision :: ss
ss=0.25d0*dble((q_spin-1)*(q_spin-1))
localHam_max=0d0
do j=1,9
  do i=0,3
  do k=0,3
    if(k==i) localHam_max = localHam_max + ss * dsqrt(dabs(DREAL(dconjg(xlocal_Ham(k,i,j))*xlocal_Ham(k,i,j))))
  enddo
  enddo
enddo
!!
!!
localHam_max = 2d0*localHam_max / c_count /dble(size_of_local_matrix)
...
implicit none
if(allocated(xlocal_Ham))deallocate(xlocal_Ham)
if(allocated(llocal_Ham))deallocate(llocal_Ham)
